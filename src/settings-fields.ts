...
export const SETTINGS_FIELDS: FieldDef[] = [
  ...
  },
  {
    ...
    label: "地图",
    kind: "catalog",
    hint: "启动时载入的地图。选了 R5F 模式后，清单会收敛到该模式声明的地图；否则用当前版本 platform/r5f_map_names.txt。",
    spec: "从清单选择，或手动输入地图 stem",
  ...
  },
  {
    ...
    label: "模式（playlist）",
    kind: "catalog",
    hint: "模式/播放列表 id。选自 R5F 模式目录（按家族分组，1v1 在最前）；留空表示启动后由玩家选择。运行中换模式用 mode set（bridge_setmode）。",
    spec: "从 R5F 模式目录选择，或手动输入 playlist id",
    scope: "重启服务器后生效；运行中切换用 `mode set`",
  ...
  },
  {
    ...
    label: "可见性",
    kind: "enum",
    hint: "spire_host_visibility：离线=只能直连 IP；隐藏=不列出但可加入；公开=出现在服务器列表。",
    spec: "0 / 1 / 2",
    scope: "重启服务器后生效",
  ...
  },
  {
    ...
    label: "在线认证",
    kind: "enum",
    hint: "sv_onlineAuthMode：官方在线认证强度，公开服建议开启。",
    spec: "0 / 1 / 2",
    scope: "重启服务器后生效",
  ...
  },
  {
    ...
    label: "1v1 数据上报",
    kind: "enum",
    hint: "fs_stats_url：R5F 的 1v1 对战统计会 POST 到 play.r5flowstate.org（引擎默认）。置空即关闭上报。",
    spec: "保持默认 / 关闭",
    scope: "重启服务器后生效",
    display: (s) => (s.statsUpload === "off" ? "关闭（不外发）" : "保持引擎默认（上报到 play.r5flowstate.org）"),
    defaultText: () => "保持引擎默认",
    ...
    options: () => [
      {
        value: "default",
        label: "保持引擎默认",
        note: "1v1 统计发往 play.r5flowstate.org/stats/1v1/ingest",
      },
      {
        value: "off",
        label: "关闭上报",
        note: '启动参数追加 +fs_stats_url ""（引擎自述：置空即关闭）',
      },
      { value: MANUAL_OPTION, label: "（手动输入…）" },
    ],
    parse: (raw) => {
  ...
  },
  {
    ...
    label: "密码",
    kind: "password",
    hint: "sv_password：留空表示不需要密码。会明文写在 r5-server.json 里。",
    spec: "任意字符，留空 = 无密码",
    scope: "重启服务器后生效",
  ...
  },
  {
    ...
    label: "命令配额",
    kind: "int",
    hint: "sv_quota_stringCmdsPerSecond：每秒钟允许的 string 命令上限，防止刷屏/滥用。",
  ...
  },
  {
    ...
    label: "脚本配额",
    kind: "int",
    hint: "sv_quota_scriptExecsPerSecond：每秒钟允许执行的脚本命令上限。",
    spec: "0–100000（0 = 关闭限流）",
    scope: "重启服务器后生效",
  ...
  },
  {
    ...
    label: "日志保留份数",
    kind: "int",
    hint: "保留最近几次运行的日志分片（logs/<版本>-<端口>-<时间>.log）。这是本机工具设置，不传给引擎。",
    spec: "1–1000",
    scope: "下次启动时清理旧分片",
  ...
  },
  {
    ...
    label: "附加参数",
    kind: "text",
    hint: "原样追加到启动命令行，用来传本表没有的参数（例如 -devsdk、+some_cvar 1）。",
    spec: "整行原样传递，留空 = 不追加",
    scope: "重启服务器后生效",
