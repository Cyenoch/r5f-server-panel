...
Windows x64 release 已在真机实跑通过（2026-09-18，Windows 11 Pro / Ryzen 7 9700X）：产物 `dist/r5-server.exe`，**110159360 B**，SHA-256 `dad06aa7…a884`，图载荷摘要 `6a2e51a0…b157`，入口 `B:/~BUN/root/app.js` + worker `B:/~BUN/root/worker.js`。同一构建链的上一次产物（`a2e54758…fee5`，只少一项界面功能）`llvm-objdump -p` 的导入表只有系统 DLL（`kernel32`/`d3d11`/`dcomp`/`dwrite`/`icuuc` 等），**不需要 VC++ 运行库，也不依赖相邻 Bun 或 JS**：把 exe 单独复制成带中文与空格的 `单文件 面板.exe`、`PATH` 只留 Windows 自带目录（不含 Bun / cargo / Git / LLVM / Python）启动，窗口完整渲染、内嵌 JS 与 worker 落下了数据根。当前产物在真机上另跑了 80 秒窗口观测（零 PowerShell 弹窗）与无障碍树点击验证。构建机与目标同为 `x86_64-pc-windows-msvc` 时**不需要 `R5_BUILD_BASE`**；序列化器由固定提交现场构建（`--revision` = `1.4.0-debug+34cbb9a40`）。细节与踩坑见 [docs/solid-gpui-notes.md](docs/solid-gpui-notes.md)。
