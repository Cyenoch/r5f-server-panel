...
┌ 标题栏 ────────────────────────────────────────────────────────┐
│ 服务器管理 ── 首页                                             │
│              服务器 ── 服务器列表 / 实例 / 实时日志 / 玩家列表 │
│              配置  ── 服务器配置 / 模式与地图 / 公告 / 主机配置│
│              运维   ── 体检 / 封禁名单 / 启动引导              │
├────────────────────────────────────────────────────────────────┤
│ 状态栏：当前版本 · 在线人数 · 端口 · 动作记录                  │
└────────────────────────────────────────────────────────────────┘
```

界面约定：**一行只留一个 ⋯ 菜单**（玩家、模式、档案、公告行），表单一律进二级弹窗
（封禁 / 解封 / 加机器人 / 改设置项 / 编辑公告 / 立即切换模式），长解释进悬停提示或折叠区。
一行动作不进正文，是刻意的：同一批事实在三个页面上各写一份，读者只会对不上口径。

| 页面       | 内容                                                                         | 会发给引擎/系统的动作                    |
| ---------- | ---------------------------------------------------------------------------- | ---------------------------------------- |
| 首页       | 实例状态、首次使用清单、本次运行结论、日志尾部                               | 启动（标题栏）/ 去日志                   |
| 服务器列表 | 本机所有 `r5f-dedi-*` 版本与体积；**启动对话框**在这里选配置档案与一次性覆盖 | 切换版本、以某档案启动                   |
| 实例       | 进程指标（运行时长/人数/内存/CPU/地图）+ 折叠的排障信息（pid/端口/监听地址） | 重启 / 停止 / 停止全部                   |
| 实时日志   | 引擎日志尾部（可按运行分片回看）+ 控制台输入与回执                           | 任意控制台命令                           |
| 玩家列表   | `status` 解析出的玩家表，机器人单独标记；行内踢 / 封禁，解封与加机器人走弹窗 | 踢 / 封禁 / 解封 / 加机器人 / 清空机器人 |
| 服务器配置 | 配置档案（⋯ 菜单：启用/覆盖/删除，新建走弹窗）+ 启动设置清单（点行改值）     | 写回设置、切换生效档案                   |
| 模式与地图 | 从版本目录读出的模式家族与地图；行内 ⋯ 菜单（设为启动模式 / 立即切换…）      | `bridge_setmode`（立即切换弹窗里选地图） |
| 公告       | `chat_announcements.csv` 概览表（⋯ 菜单改/删，新增走弹窗）+ 广播             | 保存文案、立即广播                       |
| 主机配置   | 系统侧五项能力清单（唯一出处）+ 内存/磁盘/端口占用                           | 调 `setup`（弹 UAC，先确认）             |
| 体检       | 本次运行的错误结论 + 这台机器还差哪几项（细节在主机配置页）                  | 只读                                     |
| 封禁名单   | 本机封禁台账（行内解封 / 复制玩家 ID）+ 引擎 `banlist.json` 原文（折叠）     | `banlist_reload`                         |
| 启动引导   | 首次开服清单：版本 / 服务器名 / 公网地址 / 端口与防火墙                      | 保存设置、放行端口（UAC）                |

面板的写操作全部走 CLI 已有的实现：日志、控制通道、cfg 同步、回执分类都是同一份代码，因此命令行与图形界面的行为一致。**只有 CLI 与图形面板两处入口，没有第三种状态**：`r5-server.json` 仍是唯一的生效配置来源。
