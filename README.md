...
├ 首页                                                           │
│ 服务器管理 ── 首页                                              │
│              服务器 ── 服务器列表 / 实例 / 实时日志 / 玩家列表 / 控制面板
│              配置文件 ── 服务器配置 / 模式与地图 / 公告 / 主机配置
│              运维 ── 体检 / 封禁名单 / 启动引导                  │
├────────────────────────────────────────────────────────────────┤
│ 状态栏：当前版本 · 在线人数 · 端口 · CPU/内存/带宽 · 动作记录    │
└────────────────────────────────────────────────────────────────┘
```

| 页面       | 内容                                                                         | 会发给引擎/系统的动作                                             |
| ---------- | ---------------------------------------------------------------------------- | ----------------------------------------------------------------- |
| 首页       | 实例状态、首次使用清单、最近动作                                             | 启动 / 停止 / 重启                                                |
| 服务器列表 | 本机所有 `r5f-dedi-*` 版本与体积；**启动对话框**在这里选配置档案与一次性覆盖 | 切换版本、以某档案启动                                            |
| 实例       | 进程指标（运行时长/人数/内存/CPU/地图）、pid、端口、控制通道、监听地址       | 重启 / 停止 / 停止全部                                            |
| 实时日志   | 引擎日志尾部（可按运行分片回看）+ 控制台输入与回执                           | 任意控制台命令                                                    |
| 玩家列表   | `status` 解析出的玩家表，机器人单独标记                                      | 踢 / 封禁 / 解封 / 加机器人 / 清空机器人                          |
| 控制面板   | 模式热切换、机器人、公告轮播与广播、危险操作                                 | `bridge_setmode` / `sv_addbot` / `spawnbots` / 广播 / 重启 / 停止 |
| 服务器配置 | 配置档案（新建/切换/覆盖/删除）+ 全部启动设置（声明表驱动）                  | 写回设置、切换生效档案                                            |
| 模式与地图 | 从版本目录读出的模式家族与地图                                               | 设为启动模式/地图、立即热切                                       |
| 公告       | `chat_announcements.csv` 编辑与广播                                          | 保存文案、立即广播                                                |
| 主机配置   | 防火墙 / 页面文件 / Defender / 计划任务 / 电源计划能力清单                   | 调 `setup`（弹 UAC）                                              |
| 体检       | 本次运行健康（`error/warning/script_warning`）+ 主机事实                     | 只读                                                              |
| 封禁名单   | 本机封禁台账 + 引擎 `banlist.json` 原文                                      | `banlist_reload`                                                  |
