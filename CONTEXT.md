...
5. **Windows 脚本 ASCII**：`*.bat`/`*.ps1` 内容保持 ASCII（中文 Windows 的 ANSI 解析会毁 UTF-8 无 BOM）。
6. **无 `any`**：`unknown` + 类型守卫；静态映射用 `Record`；已发布契约用具名类型。
7. **版本目录尽量只读**：只写引擎自写文件与用户明确要编辑的 `chat_announcements.csv`。
8. **不新增公网管理面**：控制通道只回环；远程运维走 SSH/私网隧道（另议）。
